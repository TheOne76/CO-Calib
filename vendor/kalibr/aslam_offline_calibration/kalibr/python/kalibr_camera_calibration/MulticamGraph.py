...
#############################################################    
    ...
    def getInitialGuesses(self, cameras, fixIntrinsics=False):
        ...
        #################################################################
        success, baselines = kcc.solveFullBatch(cameras, baselines, self, fixIntrinsics=fixIntrinsics)
    ...
    def getTargetPoseGuess(self, timestamp, cameras, baselines_HL=[]):
        ...
        if len(camids) == 0:
            sm.logWarn("getTargetPoseGuess: no camera observations available at timestamp {0}".format(timestamp))
            return None
        ...
        try:
            ...
            success = False
            T_t_cN = sm.Transformation()
               
        if not success:
            sm.logWarn("getTargetPoseGuess: solvePnP failed with solution: {0}".format(T_t_cN))
            if _use_default_pose_on_pnp_fail():
                return _default_identity_pose("getTargetPoseGuess")
            return None
        try:
            T_t_cN_mat = np.asarray(T_t_cN.T(), dtype=np.float64)
        except Exception:
            sm.logWarn("getTargetPoseGuess: failed to materialize target pose guess at timestamp {0}".format(timestamp))
            if _use_default_pose_on_pnp_fail():
                return _default_identity_pose("getTargetPoseGuess/materialize")
            return None
        if not np.all(np.isfinite(T_t_cN_mat)):
            sm.logWarn("getTargetPoseGuess: solvePnP returned non-finite pose at timestamp {0}".format(timestamp))
            if _use_default_pose_on_pnp_fail():
                return _default_identity_pose("getTargetPoseGuess/nonfinite")
            return None
        ...
        try:
            T_t_c0_mat = np.asarray(T_t_c0.T(), dtype=np.float64)
        except Exception:
            sm.logWarn("getTargetPoseGuess: failed to materialize transformed pose guess at timestamp {0}".format(timestamp))
            return None
        if not np.all(np.isfinite(T_t_c0_mat)):
            sm.logWarn("getTargetPoseGuess: transformed pose guess became non-finite at timestamp {0}".format(timestamp))
            return None
