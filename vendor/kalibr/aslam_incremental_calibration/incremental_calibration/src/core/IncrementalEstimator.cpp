...
/******************************************************************************/
    ...
    IncrementalEstimator::ReturnValue
      ...
      // check that the optimized state is finite before deciding whether to
      // keep the new batch. If a single batch drives any design variable to
      // NaN/Inf we want to reject it and restore the previous state so that
      // later frames can still be processed.
      bool nonFiniteState = false;
      std::ptrdiff_t nonFiniteDvIdx = -1;
      ...
      for (size_t i = 0; i < numDvs; ++i) {
        ...
        dv->getParameters(params);
        if (!params.allFinite()) {
          nonFiniteState = true;
          nonFiniteDvIdx = static_cast<std::ptrdiff_t>(i);
          break;
        }
      }
      ...
      if (_options.checkValidity && (srv.iterations ==
          _optimizer->options().maxIterations || !std::isfinite(srv.JStart) ||
          !std::isfinite(srv.JFinal) || srv.JFinal >= srv.JStart))
        solutionValid = false;

      // compute the information gain
      const double svLog2Sum = linearSolver->getSingularValuesLog2Sum();
      ret.informationGain = 0.5 * (svLog2Sum - _svLog2Sum);
      if (!std::isfinite(svLog2Sum) || !std::isfinite(ret.informationGain))
        solutionValid = false;
      if (nonFiniteState) {
        solutionValid = false;
        if (_options.verbose)
          std::cerr << "IncrementalEstimator::addBatch(): rejecting batch "
            "because design variable " << nonFiniteDvIdx
            << " became non-finite" << std::endl;
      }
